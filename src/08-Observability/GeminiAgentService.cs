...
public class GeminiAgentService
{
    ...
    /// <summary>
    /// Extrai dados de livros do JSON interceptado usando o Gemini.
    /// </summary>
    /// <param name="jsonData">O JSON interceptado da API.</param>
    /// <param name="schema">O schema da classe BookInfo.</param>
    /// <returns>Lista de livros extraídos pelo Gemini.</returns>
    public async Task<List<BookInfo>> ExtractBooksFromJsonAsync(string jsonData, string schema)
    {
        var prompt = BuildJsonPrompt(jsonData, schema);
    ...
    {
        var prompt = BuildHtmlPrompt(pageHtml, schema);
        var response = await CallGeminiAsync(prompt);
        return ParseBooksFromResponse(response);
    }

    private string BuildJsonPrompt(string jsonData, string schema)
    {
        return $"""
            Você é um agente especializado em extração e transformação de dados JSON.
            
            ## Tarefa
            Analise o JSON fornecido abaixo (interceptado de uma API de livros) e extraia todas as informações de livros.
            
            ## Schema de Saída
            Retorne os dados transformados no formato JSON seguindo exatamente este schema C#:
            ...
            2. Extraia todos os livros que encontrar no JSON.
            ...
            6. O campo "Key" deve conter o identificador único do livro.
            ...
            ## Resposta
            Retorne apenas o array JSON com os livros extraídos e transformados:
            """;
    }

    private string BuildHtmlPrompt(string pageHtml, string schema)
    {
        return $"""
